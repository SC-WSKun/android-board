...
// 临时保存的文件路径
const AUDIO_FILE_PATH = FileSystem.cacheDirectory + 'tts_stream_audio.mp3';
...
export async function parseResponse(res: ArrayBuffer) {
  otherLog.info('--------------------------- response ---------------------------');
  ...
  if (messageType === 0xb) { // audio-only server response
    ...
    otherLog.error(`               Error message: ${new TextDecoder().decode(errorMsg)}`);

    return true;
  } else if (messageType === 0xc) { // frontend message
    ...
    otherLog.info(`            Frontend message: ${new TextDecoder().decode(frontendPayload)}`);
    return false;
  } else {
